...
def nix_header_root():
    root = os.environ.get("SIMPLE_LINUX_HEADERS", "").strip()
    return root if root and os.path.isdir(root) else None


def explicit_headers_from_env():
    root = nix_header_root()
    if not root:
        return None
    return [
        os.path.join(root, "linux/input.h"),
        os.path.join(root, "linux/input-event-codes.h"),
        os.path.join(root, "linux/uinput.h"),
    ]


def create_ecodes(headers=None, reproducible=False):
    if headers is None:
        headers = explicit_headers_from_env()
...
include_dirs = [nix_header_root()] if nix_header_root() else []
setup(
    ext_modules=[
        Extension("evdev._input", sources=["src/evdev/input.c"], extra_compile_args=cflags, include_dirs=include_dirs),
        Extension("evdev._uinput", sources=["src/evdev/uinput.c"], extra_compile_args=cflags, include_dirs=include_dirs),
        Extension("evdev._ecodes", sources=["src/evdev/ecodes.c"], extra_compile_args=cflags, include_dirs=include_dirs),
    ],
    cmdclass={
        "build_ext": build_ext,
