...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_tabletop_handover_mp")
class G1WholebodyTabletopHandoverMP(Task):
    uid:str = "g1_wholebody_tabletop_handover_mp"
    label: str = "G1 Wholebody Tabletop Handover MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "pick up the object, then hand it to the left hand.",
            ]
        ),
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.96, 0.0,0.0], high=[-1.03, 0.0,0.0]),

            target_region=Box(low=[-0.6, -0.0], high=[-0.65, 0.05]),
            # container_region=Box(low=[-0.4, 0.05], high=[-0.45, 0.12]),
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene15",
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.62, 0.3], high=[-0.6, 0.4])
        ...
        return [
            ...
            # WalkSpec("walk to target",vx=0.35, target_yaw=0, target_distance=0.6),
            # StandSpec("initialize",),
