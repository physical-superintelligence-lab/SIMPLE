...
_PLACE_HEIGHT = 0.3
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant7_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant7MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant7_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 7 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0, 0.8,], high=[0, 0.85]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[-0.04, 0.45], high=[0.02, 0.5]),
            container_region=Box(low=[0.52, 3.33], high=[0.56, 3.37]),
            ...
            distractors_region=[
                ...
                Box(low= [-0.6,3.5], high=[0.6,3.7])   # distractor_3 on table2
            ],
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_position=Box(low=[-1, -0.0], high=[-1, 0.0]),
    ...
    def decompose(self):
        ...
        place_origon = Box(low=[0.1,-0.05], high=[0.15,0.03])
        place_position = container_pos + np.array(place_origon.sample()+ [_PLACE_HEIGHT])

        import transforms3d as t3d
        place_orientation = t3d.euler.euler2quat(0, 0, -70)
        ...
        return [
            ...
            LiftSpec("lift", up=0.2, grasp_type="bodex", hand_uid="dex3_right"),
            TurnSpec("turn_90", vx=0.1, target_yaw=0),
            TurnSpec("turn_180", vx=0.1, target_yaw=1.57),
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=1.57, target_distance=2.15),
