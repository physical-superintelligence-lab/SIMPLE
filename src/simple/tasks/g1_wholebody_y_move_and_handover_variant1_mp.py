...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_y_move_and_handover_variant1_mp")
class G1WholebodyYMoveAndHandoverVariant1MP(Task):
    uid:str = "g1_wholebody_y_move_and_handover_variant1_mp"
    label: str = "G1 Wholebody Y Move and Handover Variant 1 MP"
    description: str = "A task where the G1 robot must move to a target position and pick up an object using right hand and hand it to the left hand."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.8, 0.45,0.0], high=[-0.82, 0.5,0.0]),

            target_region=Box(low=[-0.35, -0.0], high=[-0.4, 0.06]),
            container_region=Box(low=[-0.1, 0.05], high=[-0.15, 0.12]),

            distractors_region=Box(low=[0.3, -0.3], high=[-0.5, 0.3]),
            target_stable_indices = [0],
            target_rotate_z = Box(low=-0.05, high=0.05),
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene17",
       ...
       # lazy init instance variables
        ...
        #HACK THIS task is only in scene15
    ...
    def compute_reward(self, info: dict[str, Any] ,*args, **kwargs) -> float:
        is_object_contacted_by_hand = self.check_wether_handover_success(*args, **kwargs)

        if  is_object_contacted_by_hand:
           self.reward += 0.015
        else:
            self.reward = 0  
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.39, 0.2], high=[-0.45, 0.3])
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.1, vy=-0.35,target_yaw=0.0, target_distance=0.9),
            StandSpec("initialize",),
