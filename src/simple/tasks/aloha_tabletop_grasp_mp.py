...
@TaskRegistry.register("aloha_tabletop_grasp_mp")
class AlohaTabletopGraspTaskMP(Task):
    

    uid: str = "aloha_tabletop_grasp_mp"
    label: str = "Aloha Tabletop Grasp Task"
    description: str = "A task where the Aloha robot must grasp target object on a tabletop."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        target = TargetDRCfg(
            asset_id="graspnet1b:63" # e.g., "primitive:cube"
        ),
        ...
        lighting = LightingDRCfg(
            ...
            light_num=(2,3),
            light_color_temperature=Box(low=4001, high=6001),  # I was not joking :)
    ...
    def __init__(
        self, 
        robot_uid: str = "aloha",
        target_object: str | None = "graspnet1b:63", # "graspnet1b:63"
    ...
    def decompose(self):
        ...
        return [
            OpenGripperSpec("init", hand_uid="left"),
            GraspObjectSpec("approach", target_uid=self.target.uid, pregrasp=False, hand_uid="left"),
            CloseGripperSpec("grasp", hand_uid="left"),
            LiftSpec("lift", up=0.1, hand_uid="left"),
        ]