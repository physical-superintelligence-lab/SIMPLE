...
_PLACE_HEIGHT = 0.3
@TaskRegistry.register("g1_wholebody_pick_and_bend_place_mp")
class G1WholebodyPickAndBendPlaceMP(Task):
    uid:str = "g1_wholebody_pick_and_bend_place_mp"
    label: str = "G1 Wholebody Pick and Bend Place MP"
    description: str = "A task where the G1 robot must pick an object from table1, locomote to target position, and place it ."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.0, 1.0], high=[-0.0, 1.05]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[-0.05, 0.65], high=[0.01, 0.69]),
    ...
    def decompose(self):
        ...
        place_origon = Box(low=[-0.1, -0.24], high=[0.1, -0.15])
        ...
        return [
            ...
            LiftSpec("lift", up=0.03, grasp_type="bodex", hand_uid="dex3_right"),
            TurnSpec("turn_90", vx=0.1, target_yaw=-3.14),
            TurnSpec("turn_180", vx=0.1, target_yaw=1.57),
            WalkSpec("walk_to_table2", vx=0.35, vy=0.1, target_yaw=1.57, target_distance=2),
            # StandSpec("stand_at_table2"),
            PhaseBreakSpec("phase_break_before_place", grasp_type="bodex"),
            TurnSpec("turn_90", vx=0.1, target_yaw=-3.14),
            # WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=-3.14, target_distance=0.02),
