...
_LIFT_HEIGHT = 0.02
@TaskRegistry.register("g1_inspire_wholebody_locomotion_mp")
class G1InspireWholebodyLocomotionTaskMP(Task):
    uid:str = "g1_inspire_wholebody_locomotion_mp"
    label: str = "G1 Inspire Wholebody Locomotion Task"
    description: str = "A task where the G1 Inspire robot must locomote in a wholebody manner."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, walk to the end position.",
            ]
        ),
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1, 0.0,0.0], high=[-1, 0.0,0.0]),

            target_region=Box(low=[-0.48, -0.05], high=[-0.56, 0.06]),
    ...
    def __init__(
       ...
       # lazy init instance variables
        ...
        #HACK This task only use scene2
    ...
    def decompose(self):
        ...
        return [
            ...
            PhaseBreakSpec("phase_break_before_walk"),
            TurnSpec("turn",vx=0.1,target_yaw=1.57),

            WalkSpec("walk",vx = 0.35,vy=0,target_yaw=1.57,target_distance=0.7),
