...
@TaskRegistry.register("g1_wholebody_xmove_pick_teleop")
class G1WholebodyXMovePickTaskTeleop(Task):
    uid: str = "g1_wholebody_xmove_pick_teleop"
    label: str = "G1 TELEOP XMove Pick"
    description: str = (
        "A task where the G1 robot must move forward to pick up an object ."
    )
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.9, -0.0, 0.0], high=[-1.0, 0.0, 0.0]),
            ...
            # articulated_region=Box(low=[-0.3, -0.25], high=[-0.24, -0.25]),
            # articulated_rotate_z= Box(low=-0.15, high=0.15),
        ),
        camera=CameraDRCfg(
