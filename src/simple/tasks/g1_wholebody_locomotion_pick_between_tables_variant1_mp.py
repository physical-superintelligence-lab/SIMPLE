...
_PLACE_HEIGHT = 0.25
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant1_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant1MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant1_mp"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.65, 0.0,], high=[0.7, 0.0]),
            robot_orientation_region=Box(low=[0, 0, 0, 1], high=[0, 0, 0, 1]),

            target_region=Box(low=[0.35, 0.03], high=[0.39, 0.1]),
            ...
            distractors_region=[
                ...
                Box(low=[1.7, -0.3], high=[1.9, 0.3])   # distractor_3 on table2
            ],
            target_stable_indices = [0],
            target_rotate_z = Box(low=2.57, high=3.16),
            obj_surface_map={
                "target": "table",
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_position=Box(low=[0.1, 0.0], high=[0.1, 0.0]),
            # table_height=Box(low=0.0, high=0.0),
            # rotation_z=Box(low=0, high=3.14),  # Rotation around the Z-axis
            table_height=Box(low=0.0, high=0.0),
            table2_height=Box(low=-0.00, high=-0.01),
            room_choices=["hssd:scene9"],
            scene_manager="hssd",
            enable_table2=True,
        ),
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene9",
    ...
    def decompose(self):
        ...
        place_origon = Box(low=[1.5, -0.1], high=[1.6, 0.1])
        place_position =np.array(place_origon.sample()+ [_PLACE_HEIGHT])
        ...
        return [
            ...
            # ),
            # LowerSpec("lower", down=0.04, grasp_type="bodex"),
            # HeightAdjustSpec("height_adjust", height=-0.05, grasp_type="bodex", keep_waist_pose=True),
