...
_PLACE_HEIGHT = 0.2
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant12_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant12MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant12_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 12 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.82, 0.05], high=[-0.85, 0.05]),
            # robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[-0.45, 0.01], high=[-0.5, 0.06]),
            ...
            target_rotate_z = Box(low=-0.57, high=0),
            obj_surface_map={
                "target": "table",
                "container": "table2",
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=-3.14, target_distance=0.2),
