...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_y_move_and_pick_variant2_mp")
class G1WholebodyYMoveAndPickVariant2MP(Task):
    uid:str = "g1_wholebody_y_move_and_pick_variant2_mp"
    label: str = "G1 Wholebody Y Move and Pick Variant 2 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.8, 1.05,0.0], high=[0.85, 1.1,0.0]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[0.35, 0.6], high=[0.37, 0.65]),
    ...
    def __init__(
       ...
       # lazy init instance variables
        ...
        #HACK This task only in scene11
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.1,vy=-0.35, target_yaw=0, target_distance=0.9),
