...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_y_move_and_bend_pick_mp")
class G1WholebodyTurnYMoveAndBendPickMP(Task):
    uid:str = "g1_wholebody_turn_y_move_and_bend_pick_mp"
    label: str = "G1 Wholebody Turn Y Move and Bend Pick MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.45, 1.26,0.35], high=[0.5, 1.28,0.35]),
            robot_orientation_region = Box(
                low = [0.342, 0, 0, -0.940], 
                high= [0.940, 0, 0, -0.342]
            ),

            target_region=Box(low=[0.09, 0.84], high=[0.11, 0.88]),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_height=Box(low=0.00, high=0.00),
            room_choices=["hssd:scene46"],
            scene_manager="hssd"
        ),
    ...
    def __init__(
       ...
       # lazy init instance variables
        ...
        #HACK THIS task is only in scene46
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.35, -0.05], high=[-0.32, -0])
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.1,vy=-0.35, target_yaw=-1.57, target_distance=0.9),
