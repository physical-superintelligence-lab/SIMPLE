...
_PLACE_HEIGHT = 0.3
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_tabletop_pick_n_place_mp")
class G1TabletopPickNPlaceMP(Task):
    uid:str = "g1_tabletop_pick_n_place_mp"
    label: str = "G1 Tabletop pick and place MP"
    description: str = "A task where the G1 robot must pick up a target object and place it into a container."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["12","46","6"]  # Exclude the target object and the container
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.8, -0.0], high=[-0.8, -0.0]),
            target_region=Box(low=[-0.35, -0.04], high=[-0.55, 0.04]),
            container_region=Box(low=[-0.2, -0.09], high=[-0.2, 0.01]),
    ...
    def clone_layout(self, options: dict[str, Any]) -> None:
        ...
        self._layout = self.dr.replicate_env(self.robot, self.sensor_cfgs, deepcopy(options), self.split) # type: ignore
    ...
    def decompose(self):
        ...
        return [
            ...
            GraspObjectSpec("approach", target_uid=self.target.uid, pregrasp=False,grasp_type="bodex",hand_uid="dex3_right"),
