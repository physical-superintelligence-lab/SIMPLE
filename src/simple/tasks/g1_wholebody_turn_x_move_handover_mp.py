...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_x_move_and_handover_mp")
class G1WholebodyTurnXMoveAndHandoverMP(Task):
    uid:str = "g1_wholebody_turn_x_move_and_handover_mp"
    label: str = "G1 Wholebody Turn X Move and Handover MP"
    description: str = "A task where the G1 robot must turn and move to a target position and hand over it to the left hand"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, turn and move to the target position and pick up the object,and then hand over it to the left hand.",
            ]
        ),
        ...
        spatial = SpatialDRCfg(
            ...
            robot_orientation_region = Box(
                low = np.array([0.96592583, 0.0, 0.0, -0.3], dtype=np.float32),
                high= np.array([1.00000000, 0.0, 0.0,  0.3], dtype=np.float32)
            ),

            target_region=Box(low=[-0.39,-0.16], high=[-0.35, -0.13]),
            container_region=Box(low=[-0.1, 0.2], high=[-0.15, 0.28]),
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.4, -0.05], high=[-0.35, -0])
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=0, target_distance=0.51),
            StandSpec("initialize",),
            ...
            LiftSpec("lift", height =0.04, grasp_type="bodex",hand_uid="dex3_right"),
