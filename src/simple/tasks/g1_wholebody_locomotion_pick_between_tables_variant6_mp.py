...
_PLACE_HEIGHT = 0.35
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant6_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant6MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant6_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 6 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[1.0, 0.95], high=[1.0, 1.0]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[0.94, 0.6], high=[1.02, 0.65]),
            container_region=Box(low=[1.3, 3.1], high=[1.35, 3.15]),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_position=Box(low=[0.8, 0.25], high=[0.8, 0.25]),
    ...
    def decompose(self):
        ...
        place_origon = Box(low=[-0.24, -0.1], high=[-0.15, -0.02])
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=1.57, target_distance=1.73),
