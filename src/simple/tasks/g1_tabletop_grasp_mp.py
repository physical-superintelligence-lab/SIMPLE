...
@TaskRegistry.register("g1_tabletop_grasp_mp")
class G1TabletopGraspMP(Task):
    uid: str = "g1_tabletop_grasp_mp"
    label: str = "G1 Tabletop Grasp MP"
    description: str = (
        "A task where the G1 robot must grasp target object on a tabletop."
    )
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial=SpatialDRCfg(
            ...
            target_region=Box(low=[-0.3, -0.07], high=[-0.55, 0.05]),
    ...
    def decompose(self):
        from simple.datagen.subtask_spec import (  # MoveEEFToPoseSpec,
            CloseGripperSpec,
            GraspObjectSpec,
            LiftSpec,
            OpenGripperSpec,
