...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_pick_mp")
class G1WholebodyTurnPickMP(Task):
    uid:str = "g1_wholebody_turn_pick_mp"
    label: str = "G1 Wholebody Turn Pick MP"
    description: str = "A task where the G1 robot must turn and pick up an object."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.37, 0.21], high=[-1.33, 0.21]),
            ...
            target_region=Box(low=[-0.93,0.21], high=[-0.9, 0.21]),
            container_region=Box(low=[-0.6, 0.2], high=[-0.65, 0.28]),
