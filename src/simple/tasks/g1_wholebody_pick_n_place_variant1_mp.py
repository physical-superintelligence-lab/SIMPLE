...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_pick_n_place_variant1_mp")
class G1WholebodyPickNPlaceVariant1MP(Task):
    uid:str = "g1_wholebody_pick_n_place_variant1_mp"
    label: str = "G1 Wholebody Pick and Place Variant 1 MP"
    description: str = "A task where the G1 robot must pick a box from the end of table and place it on the container."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, pick up the red box and place it on the container.",
            ]
        ),
