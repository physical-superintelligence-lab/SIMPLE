...
_PLACE_HEIGHT = 0.4
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_x_move_bend_pick_n_place_mp")
class G1WholebodyXMoveBendPickNPlaceMP(Task):
    uid:str = "g1_wholebody_x_move_bend_pick_n_place_mp"
    label: str = "G1 Wholebody X Move Bend Pick N Place MP"
    description: str = "A task where the G1 robot must move to a target position and then bend and pick up an object, and then place it on a target position."
    ...
    sensor_cfgs: dict[str, SensorCfg] = dict(
        front_stereo = StereoCameraCfg(
            ...
            pose=dict( 
                ...
                polar=np.deg2rad(20),  # polar angle θ
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, move to the target position,bend the robot and pick up the object,then place it on the container",
            ]
        ),
       ...
       container = TargetDRCfg(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1, -0.0,0.35], high=[-1.05, 0.0,0.35]),

            target_region=Box(low=[-0.27 ,-0.00], high=[-0.27, 0.03]),

            container_region=Box(low=[-0.11 ,0.08], high=[-0.13, 0.12]),
    ...
    def decompose(self):
        ...
        place_origon = Box(low=[-0.1, -0.24], high=[-0.1, -0.18])
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=0, target_distance=0.55),
            ...
            # WalkSpec("walk_to_container",  vx= 0.35,target_yaw=0, target_distance=0.08),
