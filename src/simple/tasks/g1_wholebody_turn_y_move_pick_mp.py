...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_y_move_and_pick_mp")
class G1WholebodyTurnYMoveAndPickMP(Task):
    uid:str = "g1_wholebody_turn_y_move_and_pick_mp"
    label: str = "G1 Wholebody Y Move and Pick MP"
    description: str = "A task where the G1 robot must move to a target position and pick a box from the end of table."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, turn and move to the target position and pick up the object.",
            ]
        ),

        target = TargetDRCfg(
            asset_id="graspnet1b:10" # e.g., "primitive:cube"
        ),
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["10","46"]  # Exclude the target object 
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.5, 1.05,0.0], high=[-0.6, 1.08,0.0]),
            robot_orientation_region = Box(
                low = [0.342, 0, 0, -0.940], 
                high= [0.940, 0, 0, -0.342]
            ),

            target_region=Box(low=[-0.0, 0.64], high=[-0.0, 0.68]),
            container_region=Box(low=[-0.3, -0.03], high=[-0.37, 0.08]),
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene40",
       ...
       # lazy init instance variables
        ...
        #HACK This task only in scene40
    ...
    def decompose(self):
        ...
        place_position = container_pos + np.array([-0.1, -0.1, _PLACE_HEIGHT])

        import transforms3d as t3d
        place_orientation = t3d.euler.euler2quat(0, 0, 0)

        robot_orientation = self.layout.actors["robot"].pose.quaternion
        roll, pitch, yaw = t3d.euler.quat2euler(robot_orientation, axes='wxyz')
        target_yaw = -1.57 -yaw


        return [
            StandSpec("initialize",),
            TurnSpec("turn", vx=0.1, target_yaw=target_yaw),
            
            WalkSpec("walk to target",vx=0.1,vy=0.35, target_yaw=target_yaw, target_distance=0.9),
