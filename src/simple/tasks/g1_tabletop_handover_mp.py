...
_PLACE_HEIGHT = 0.32
_LOWER_HEIGHT = 0.01
@TaskRegistry.register("g1_tabletop_handover_mp")
class G1TabletopHandoverMP(Task):
    uid:str = "g1_tabletop_handover_mp"
    label: str = "G1 Tabletop Handover MP"
    description: str = "A task where the G1 robot must handover target object from right hand to left hand."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "Hand over {} from right hand to left hand.",
                "Pass {} from right arm to left arm.",
        ...
        container = TargetDRCfg(
            asset_id="objects:0"  # or used "graspnet1b:6"
        ),
        ...
        spatial = SpatialDRCfg(
            ...
            target_region=Box(low=[-0.32, -0.18], high=[-0.24, -0.06]),
            container_region=Box(low=[-0.26, 0.1], high=[-0.20, 0.26]),

            distractors_region=Box(low=[0.3, -0.3], high=[-0.5, 0.3]),
            target_stable_indices = [0],
            target_rotate_z = Box(low=-0.01, high=0.01),
    ...
    def observation_space(self) -> spaces.Space:
        ...
        obs: dict[str, Any] = {
            ...
            # "actuators_action": spaces.Box(-np.pi, np.pi, shape=(self.robot.dof,), dtype=np.float32),
    ...
    def compute_reward(self, info: dict[str, Any], *args, **kwargs) -> float:
        is_object_in_container = self.check_object_in_container(*args, **kwargs)
        # is_object_contacted_by_hand = self.check_hand_object_contact(*args, **kwargs)

        if is_object_in_container :
            joints_names = self.robot.joint_names
            avg_error = sum(abs(self.robot.get_robot_qpos()[j] - self.robot.init_joint_states[j]) for j in joints_names) / len(joints_names)

            # reward = math.exp(-8 * avg_error)
            self.reward +=0.2
    ...
    def decompose(self):
        ...
        handover_region = Box(low=[-0.42, -0.27], high=[-0.50, -0.20])
        handover_position = np.array(handover_region.sample())

        transition_region = Box(low=[-0.46, 0.20], high=[-0.50, 0.26])
        transition_position = np.array(transition_region.sample())

        import transforms3d as t3d
        container_pos = self.container.pose.position
        random_x = np.random.uniform(-0.135, -0.115)
        random_y = np.random.uniform(0.055, 0.08)


        # random_x = np.random.uniform(-0.105, -0.05)
        # random_y = np.random.uniform(0.1, 0.12)



        place_position = container_pos + np.array([random_x, random_y, _PLACE_HEIGHT])
        place_orientation = t3d.euler.euler2quat(0, 0, 0)


        return [
            OpenGripperSpec("init",grasp_type="bodex",hand_uid="dex3_right"),
            GraspObjectSpec("approach", target_uid=self.target.uid, pregrasp=False,grasp_type="bodex",hand_uid="dex3_right",lock_links=[ "waist_roll_link","left_hand_palm_link"]),
            # CloseGripperSpec("grasp"),
            LiftSpec("lift", up=_LIFT_HEIGHT, grasp_type="bodex",hand_uid="dex3_right"),
            MoveEEFToPoseSpec("move_to_handover",                     
                            position=handover_position,
                            grasp_type="bodex",hand_uid="dex3_right",lock_links=["waist_yaw_link", "waist_roll_link", "torso_link","left_hand_palm_link", "left_hand_index_1_link","left_hand_middle_1_link","left_hand_thumb_2_link"]),
            ...
            GraspObjectSpec("approach", target_uid=self.target.uid, pregrasp=False,grasp_type="bodex",hand_uid="dex3_left",
                            lock_links=["waist_roll_link","right_hand_palm_link","right_hand_index_1_link","right_hand_middle_1_link","right_hand_thumb_2_link"],keep_force=True),
            OpenGripperSpec("init",grasp_type="bodex",hand_uid="dex3_right"),
            ...
            LiftSpec("lift", up=0.14, grasp_type="bodex",hand_uid="dex3_left"),
            MoveEEFToPoseSpec("move_to_place",                     
            ...
            OpenGripperSpec("init",grasp_type="bodex",hand_uid="dex3_left"),
            RetreatSpec("retreat",grasp_type="bodex",hand_uid="dex3_left")
        ]
