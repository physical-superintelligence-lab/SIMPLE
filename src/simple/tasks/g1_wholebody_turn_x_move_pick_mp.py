...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_x_move_and_pick_mp")
class G1WholebodyTurnXMoveAndPickMP(Task):
    uid:str = "g1_wholebody_turn_x_move_and_pick_mp"
    label: str = "G1 Wholebody Turn X Move and Pick MP"
    description: str = "A task where the G1 robot must turn and move to a target position and pick up an object."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["10"]  # Exclude the target object 
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.15, 0.2], high=[-1.2, 0.2]),
            robot_orientation_region = Box(
                low = np.array([0.96592583, 0.0, 0.0, -0.5], dtype=np.float32),
                high= np.array([1.00000000, 0.0, 0.0,  0.5], dtype=np.float32)
            ),

            target_region=Box(low=[-0.35,0.24], high=[-0.38, 0.3]),
            container_region=Box(low=[-0.2, 0.2], high=[-0.25, 0.28]),
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene7",
        target_object: str | Object = "graspnet1b:10",
    ...
    def decompose(self):
        ...
        roll, pitch, yaw = t3d.euler.quat2euler(robot_orientation, axes='sxyz')
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=target_yaw, target_distance=0.4),
            StandSpec("initialize",),
