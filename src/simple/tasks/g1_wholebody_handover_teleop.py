...
@TaskRegistry.register("g1_wholebody_handover_teleop")
class G1WholebodyHandoverTeleop(Task):
    uid:str = "g1_wholebody_handover_teleop"
    label: str = "G1 TELEOP Handover"
    description: str = "A task where the G1 robot must handover an object and place it on a container."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.75, 0,0], high=[-0.77, 0,0]),
            # robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),
            # robot_region=Box(low=[-1.15, 0.0,], high=[-1.17, 0.0]),
            target_region=Box(low=[-0.51, 0.02], high=[-0.47, -0.04]),
            container_region=Box(low=[-0.5, 0.35], high=[-0.45, 0.45]),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_height=Box(low=0.55, high=0.55),
