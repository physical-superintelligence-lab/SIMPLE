...
@TaskRegistry.register("g1_wholebody_push_office_chair_teleop")
class G1WholebodyPushOfficeChairTaskTeleop(Task):
    uid: str = "g1_wholebody_push_office_chair_teleop"
    label: str = "G1 TELEOP Push Office Chair"
    description: str = (
        "A task where the G1 robot must push the office chair."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language=LanguageDRCfg(
            instructions=[
                "move forward to the office chair and push it to the table",
            ]
        ),
        ...
        articulated = ArticulatedObjectDrCfg(
            asset_id="articulated:0", # e.g., "
        ),
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.65, 0, 0.0], high=[-1.75, 0.0, 0.0]),
            target_region=Box(low=[0.4, -0.3], high=[0.5, -0.4]),
            ...
            articulated_rotate_z= Box(low=3.14, high=3.14),
        ),
        camera=CameraDRCfg(
        ...
        scene=TabletopSceneDRCfg(
            ...
            table_position=Box(low=[1, 0], high=[1, 0]),
    ...
    def check_wether_chair_is_pushed(self, info: dict[str, Any], *args, **kwargs) -> bool:
        mujoco_env = kwargs.get("mujoco_env",None)
        if mujoco_env is  None:
            return False
        
        chair_joint0_qpos = mujoco_env.mjData.joint("articulate_floating_base").qpos[:3].copy()
        # faucet_joint1_qpos = mujoco_env.mjData.joint("articulate_joint_1").qpos[0]
        return np.linalg.norm(chair_joint0_qpos[0] - self.chair_init_pos[0]) > 0.8
        

    def compute_reward(self, info: dict[str, Any], *args, **kwargs) -> float:
        if self.chair_init_pos is None:
            mujoco_env = kwargs.get("mujoco_env",None)
            self.chair_init_pos = mujoco_env.mjData.joint("articulate_floating_base").qpos[:3].copy()# x y z

        chair_is_pushed = self.check_wether_chair_is_pushed(info, *args, **kwargs)
        if chair_is_pushed:
            self.reward +=0.06
        else:
            self.reward = 0.0
