...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_x_move_bend_handover_mp")
class G1WholebodyXMoveBendHandoverMP(Task):
    uid:str = "g1_wholebody_x_move_bend_handover_mp"
    label: str = "G1 Wholebody X-Move Bend Handover MP"
    description: str = "A task where the G1 robot must bend and pick up an object using right hand and hand it to the left hand."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0, -1,0.38], high=[-0.0, -1.05,0.38]),
            robot_orientation_region=Box(low=[0.717, 0, 0, 0.717], high=[0.717, 0, 0, 0.717]),

            target_region=Box(low=[-0.04 ,-0.38], high=[0.04, -0.4]),
            container_region=Box(low=[-0.17, 1], high=[-0.14, 2]),
            distractors_region=Box(low=[-0.2, -0.3], high=[0.4, 0.3]),
            target_stable_indices = [0],
            target_rotate_z = Box(low=1.55, high=1.57),
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.07, -0.43], high=[-0.12, -0.4])
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=1.57, target_distance=0.42),
            StandSpec("initialize",),
