...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_y_move_and_pick_mp")
class G1WholebodyYMoveAndPickMP(Task):
    uid:str = "g1_wholebody_y_move_and_pick_mp"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.4, 1.02,0.0], high=[0.6, 1.05,0.0]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[-0.0, 0.63], high=[-0.0, 0.68]),
