...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_x_move_and_bend_pick_mp")
class G1WholebodyTurnXMoveAndBendPickMP(Task):
    uid:str = "g1_wholebody_turn_x_move_and_bend_pick_mp"
    label: str = "G1 Wholebody Turn X Move and Bend Pick MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.1, -0.2,0.35], high=[-1.15, -0.2,0.35]),
            ...
            target_region=Box(low=[-0.3,-0.15], high=[-0.27, -0.1]),
