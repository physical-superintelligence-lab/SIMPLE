...
@TaskRegistry.register("vega_tabletop_find_n_grasp_mp")
class VegaTabletopFindNGraspMP(Task):
    

    uid: str = "vega_tabletop_find_n_grasp_task_mp"
    label: str = "Vega Tabletop Find and Grasp Task MP"
    description: str = "A task where the Vega robot must first actively search the target and then grasp it on a tabletop."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        target = TargetDRCfg(
            asset_id="graspnet1b:12" # 12 is apple     # e.g., "primitive:cube"
        ),
        ...
        distractors = DistractorDRCfg(
            ...
            include=["0"], # fixed cracker-box as the distrator
    ...
    def observation_space(self) -> spaces.Space:
        ...
        obs: dict[str, Any] = {
            "agent": spaces.Box(-np.pi, np.pi, shape=(self.robot.dof+4,), dtype=np.float32), # FIXME handle gripper
            "joint_qpos": spaces.Box(-np.pi, np.pi, shape=(self.robot.dof+4,), dtype=np.float32),
    ...
    def preload_objects(self) -> list[Actor]:
        ...
        for asset in asset_manager:
            yield ActorReigstry.make("object", asset)#
