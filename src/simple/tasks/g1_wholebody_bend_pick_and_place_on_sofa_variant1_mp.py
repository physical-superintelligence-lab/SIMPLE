...
_LIFT_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_bend_pick_and_place_on_sofa_variant1_mp")
class G1WholebodyBendPickAndPlaceOnSofaVariant1MP(Task):
    uid:str = "g1_wholebody_bend_pick_and_place_on_sofa_variant1_mp"
    label: str = "G1 Wholebody Bend Pick And Place On Sofa Variant 1 Task"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.6, 0.0,0.33], high=[0.66, 0.0,0.33]),
            robot_orientation_region=Box(low=[0, 0, 0, 1], high=[0, 0, 0, 1]),

            target_region=Box(low=[0.23, -0.0], high=[0.25, 0.03]),

            distractors_region=Box(low=[0.3, -0.3], high=[-0.5, 0.2]),
    ...
    def decompose(self):
        ...
        return [
            ...
            # HeightAdjustSpec("adjust_height",height=-0.15),
            HeightAdjustSpec("adjust_height",height=0),
