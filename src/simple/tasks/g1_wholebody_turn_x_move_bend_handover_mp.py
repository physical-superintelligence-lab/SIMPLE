...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_x_move_and_bend_handover_mp")
class G1WholebodyTurnXMoveAndBendHandoverMP(Task):
    uid:str = "g1_wholebody_turn_x_move_and_bend_handover_mp"
    label: str = "G1 Wholebody Turn X Move and Bend Handover MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.0, -0.2,0.35], high=[-1.05, -0.2,0.35]),
            robot_orientation_region = Box(
                low = np.array([1, 0.0, 0.0, 0], dtype=np.float32),
                high= np.array([1.00000000, 0.0, 0.0,  0.3], dtype=np.float32)
            ),

            target_region=Box(low=[-0.3,-0.23], high=[-0.27, -0.19]),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_height=Box(low=0.05, high=0.05),
            room_choices=["hssd:scene47"],
            scene_manager="hssd"
        ),
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene44",
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.35, -0.14], high=[-0.3, -0.08])
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=0, target_distance=0.52),
            ...
            HeightAdjustSpec("adjust_height",height=-0.0,keep_waist_pose=True),
            LiftSpec("lift", height =0.1, grasp_type="bodex",hand_uid="dex3_right"),
