...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_x_move_and_pick_n_place_mp")
class G1WholebodyXMoveAndPickNPlaceMP(Task):
    uid:str = "g1_wholebody_x_move_and_pick_n_place_mp"
    label: str = "G1 Wholebody X Move and Pick and Place MP"
    description: str = "A task where the G1 robot must move to a target position and pick a box from the end of table, and then place it on the another end of the table."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, walk to the target position, pick up the box and place it on the container.",
            ]
        ),
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=0, target_distance=0.3),
