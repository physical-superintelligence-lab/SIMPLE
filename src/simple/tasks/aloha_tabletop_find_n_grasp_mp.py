...
@TaskRegistry.register("aloha_tabletop_find_n_grasp_mp")
class AlohaTabletopFindNGraspTaskMP(Task):
    

    uid: str = "aloha_tabletop_find_n_grasp_mp"
    label: str = "Aloha Tabletop Find and Grasp Task"
    description: str = "A task where the Aloha robot must first actively search the target and then grasp it on a tabletop."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        target = TargetDRCfg(
            asset_id="graspnet1b:13" # 13 is lemon     # e.g., "primitive:cube"
        ),

        distractors = DistractorDRCfg(
            res_id="graspnet1b", 
            number_of_distractors=3, #3
            allow_duplicates=False,
            include=["0", "1"], # fixed cracker-box as the distrator
            exclude=["13"]  # Exclude the target object 
        ),
