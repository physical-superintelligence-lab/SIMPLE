...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_x_move_and_handover_variant1_mp")
class G1WholebodyTurnXMoveAndHandoverVariant1MP(Task):
    uid:str = "g1_wholebody_turn_x_move_and_handover_variant1_mp"
    label: str = "G1 Wholebody Turn X Move and Handover Variant 1 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.1, -0.2], high=[-1.15, -0.2]),
            ...
            target_region=Box(low=[-0.29,-0.12], high=[-0.27, -0.08]),
            container_region=Box(low=[-0.12, 0.24], high=[-0.15, 0.28]),
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.35, -0.05], high=[-0.32, 0.05])
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=0, target_distance=0.5),
            StandSpec("initialize",),
