...
_PLACE_HEIGHT = 0.4
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant13_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant13MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant13_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 13 MP"
    description: str = "A task where the G1 robot must pick an object from table1, locomote to table2, and place it in a table2."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.0, 0.98,], high=[0.0, 1.03]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),
            # robot_region=Box(low=[-1.15, 0.0,], high=[-1.17, 0.0]),
            target_region=Box(low=[0.02, 0.66], high=[-0.03, 0.7]),
            # container_region=Box(low=[0.54, 2.12], high=[0.6, 2.17]),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_height=Box(low=0.0, high=0.0),
            table2_height=Box(low=-0.2, high=-0.2),
    ...
    def compute_reward(self, info: dict[str, Any] ,*args, **kwargs) -> float:
        ...
        is_object_on_table2 = self.check_object_on_table2(info=info, *args, **kwargs)
        is_object_contacted_by_hand = self.check_hand_object_contact(*args, **kwargs)

        if is_object_on_table2 and not is_object_contacted_by_hand:
            self.reward += 0.1
        else:
            self.reward = 0.0
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0, target_yaw=3.14, target_distance=0.5),
