...
@TaskRegistry.register("g1_wholebody_pick_and_place_and_hug_container_teleop")
class G1WholebodyPickAndPlaceAndHugContainerTaskTeleop(Task):
    uid:str = "g1_wholebody_pick_and_place_and_hug_container_teleop"
    label: str = "G1 Pick and Place and Hug Container Teleop"
    description: str = "A task where the G1 robot must pick up an object and place it on a container and hug the container ,walk to the next table and place the container."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                " pick up the {} from table1,hug the container,walk to table2,and place  on table2.",
            ]
        ),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_position=Box(low=[0, -0.5], high=[0, -0.5]),
            # table_height=Box(low=0.0, high=0.0),
            # rotation_z=Box(low=0, high=3.14),  # Rotation around the Z-axis
            table_height=Box(low=0.67, high=0.67),
            table2_height=Box(low=0.67, high=0.67),
            table2_rotation_z=Box(low=1.57, high=1.57),
            table2_position=Box(low=[0.7, 1.5], high=[0.7, 1.5]),
            room_choices=["hssd:scene14"],
            scene_manager="hssd",
            enable_table2=True,
        ),
    ...
    def check_container_on_table(self, *args, **kwargs) -> bool:
        """
        Check if the container is on table2 by detecting contact with table2.
        """
        ...
        for i_contact in range(mj_physics_data.ncon):
            ...
            if (container_name in body1 and "table" == body2) or \
               (container_name in body2 and "table" == body1): 
                return True
        return False
    ...
    def decompose(self):
        ...
        return [
            ...
            TurnSpec("turn_90", vx=0.0, target_yaw=1.57),
            TurnSpec("turn_180", vx=0.0, target_yaw=3.14),
            WalkSpec("walk_to_table2", vx=0.35, vy=0, target_yaw=3.16, target_distance=0.5),
            StandSpec("stop",target_yaw=3.14),
