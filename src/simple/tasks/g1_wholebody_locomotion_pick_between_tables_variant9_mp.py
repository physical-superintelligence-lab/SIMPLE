...
_PLACE_HEIGHT = 0.2
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant9_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant9MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant9_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 9 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.5, -0.3], high=[0.5, -0.27]),
            robot_orientation_region=Box(low=[0.717, 0, 0, 0.717], high=[0.717, 0, 0, 0.717]),

            target_region=Box(low=[0.54, 0.04], high=[0.46, 0.1]),
            ...
            target_rotate_z = Box(low=0.57, high=1.57),
            obj_surface_map={
                "target": "table",
                "container": "table2",
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=-1.57, target_distance=0.19),
