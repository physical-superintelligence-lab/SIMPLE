...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_y_move_and_pick_variant1_mp")
class G1WholebodyTurnYMoveAndPickVariant1MP(Task):
    uid:str = "g1_wholebody_turn_y_move_and_pick_variant1_mp"
    label: str = "G1 Wholebody Y Move and Pick Variant 1 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.45, 0.97,0.0], high=[0.55, 1,0.0]),
            robot_orientation_region = Box(
                low = [0.342, 0, 0, -0.940], 
                high= [0.940, 0, 0, -0.342]
            ),

            target_region=Box(low=[0.08, 0.47], high=[0.12, 0.50]),
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.1,vy=-0.35, target_yaw=-1.57, target_distance=1),
            StandSpec("initialize",),
