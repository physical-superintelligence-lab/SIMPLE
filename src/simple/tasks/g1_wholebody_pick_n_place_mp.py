...
_LIFT_HEIGHT = 0.02
@TaskRegistry.register("g1_wholebody_pick_n_place_mp")
class G1WholebodyPickNPlaceMP(Task):
    uid:str = "g1_wholebody_pick_n_place_mp"
    label: str = "G1 Wholebody Pick and Place MP"
    description: str = "A task where the G1 robot must pick a box from the end of table and place it on the another end of the table."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, pick up the red box and place it on the another end of the table.",
            ]
        ),
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.85, 0.0,0.0], high=[-0.97, 0.0,0.0]),

            target_region=Box(low=[-0.48, -0.06], high=[-0.56, 0.06]),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_position=Box(low=[0.175, 0], high=[0.175, 0.0]),
    ...
    def decompose(self):
        ...
        target_distance_0_region = Box(low=0.78, high=0.82)
        target_distance_0 = round(target_distance_0_region.sample(),2)

        target_distance_1_region = Box(low=2.43, high=2.46)
        target_distance_1 = round(target_distance_1_region.sample(),2)

        target_distance_2_region = Box(low=0.7, high=0.75)
        target_distance_2 = round(target_distance_2_region.sample(),2)
        ...
        return [
            ...
            GraspObjectSpec("approach", target_uid=self.target.uid, pregrasp=False,grasp_type="bodex",hand_uid="dex3_right",lock_links=[ "waist_roll_link","torso_link","left_hand_palm_link"]),
            ...
            WalkSpec("walk",vx = 0.35,vy=0.15,target_yaw=0,target_distance=target_distance_1),
            TurnSpec("turn",vx=0.1,target_yaw=-1.57),
