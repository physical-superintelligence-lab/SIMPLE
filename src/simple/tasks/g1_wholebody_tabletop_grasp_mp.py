...
from simple.dr.manager import (DRManager,  # , LayoutManager
                               TabletopGraspDRManager)
...
_LIFT_HEIGHT = 0.02
@TaskRegistry.register("g1_wholebody_tabletop_grasp_mp")
class G1WholebodyTabletopGraspMP(Task):
    uid:str = "g1_wholebody_tabletop_grasp_mp"
    label: str = "G1 Wholebody Tabletop Grasp MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1, 0.0,0.0], high=[-1.1, 0.0,0.0]),

            target_region=Box(low=[-0.62, -0.03], high=[-0.67, 0.03]),
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_position=Box(low=[0.13, 0], high=[0.13, 0.0]),
    ...
    def decompose(self):
        from simple.datagen.subtask_spec import (GraspObjectSpec,
                                                 HeightAdjustSpec, LiftSpec,
                                                 OpenGripperSpec,
                                                 PhaseBreakSpec, StandSpec,
                                                 TurnSpec, WalkSpec)
