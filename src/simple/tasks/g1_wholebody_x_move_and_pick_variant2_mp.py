...
_PLACE_HEIGHT = 0.1
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_x_move_and_pick_variant2_mp")
class G1WholebodyXMoveAndPickVariant2MP(Task):
    uid:str = "g1_wholebody_x_move_and_pick_variant2_mp"
    label: str = "G1 Wholebody X Move and Pick Variant 2 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0, -0.67,0.0], high=[0, -0.72,0.0]),
            robot_orientation_region=Box(low=[0.717, 0, 0, 0.717], high=[0.717, 0, 0, 0.717]),

            target_region=Box(low=[0.05, 0.0], high=[-0.00, 0.06]),
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=1.57, target_distance=0.5),
