...
_PLACE_HEIGHT = 0.2
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant10_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant10MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant10_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 10 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        target = TargetDRCfg(
            asset_id="graspnet1b:12" # e.g., "primitive:cube"
        ),
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["12","46","6"]  # Exclude the target object 
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.1, 1.2], high=[0.1, 1.25]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[0.06, 0.85], high=[0.12, 0.9]),
            ...
            distractors_region=[
                ...
                Box(low=[0.1, 0.2], high=[0.6, 0.4]),  # distractor_1 on table1
                Box(low=[0.1, 0.3], high=[0.6, 0.5]),  # distractor_2 on table1
                Box(low= [0.6,3.1], high=[1.2,3.4])   # distractor_3 on table2
            ],
            target_stable_indices = [0],
            target_rotate_z = Box(low=-2.57, high=-1.57),
            obj_surface_map={
                "target": "table",
                "container": "table2",
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=1.57, target_distance=0.2),
