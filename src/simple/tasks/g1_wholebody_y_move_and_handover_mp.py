...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_y_move_and_handover_mp")
class G1WholebodyYMoveAndHandoverMP(Task):
    uid:str = "g1_wholebody_y_move_and_handover_mp"
    label: str = "G1 Wholebody Y Move and Handover MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.85, 0.45,0.0], high=[-0.87, 0.5,0.0]),

            target_region=Box(low=[-0.4, -0.0], high=[-0.45, 0.06]),
    ...
    def decompose(self):
        ...
        transition_region = Box(low=[-0.44, 0.2], high=[-0.5, 0.3])
