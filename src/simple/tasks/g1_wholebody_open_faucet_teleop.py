...
@TaskRegistry.register("g1_wholebody_open_faucet_teleop")
class G1WholebodyOpenFaucetTaskTeleop(Task):
    uid: str = "g1_wholebody_open_faucet_teleop"
    label: str = "G1 TELEOP Open Faucet"
    description: str = (
        "A task where the G1 robot must open the faucet."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        articulated = ArticulatedObjectDrCfg(
            asset_id="articulated:4", # e.g., "
        ),
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.1, 0, 0.0], high=[-1.2, 0.0, 0.0]),
            ...
            articulated_region=Box(low=[-0.75, -0.1,0.7], high=[-0.7, 0.1,0.7]),
    ...
    def check_wether_faucet_is_opened(self, info: dict[str, Any], *args, **kwargs) -> bool:
        mujoco_env = kwargs.get("mujoco_env",None)
        if mujoco_env is  None:
            return False
        
        faucet_joint0_qpos = mujoco_env.mjData.joint("articulate_joint_0").qpos[0]
        # faucet_joint1_qpos = mujoco_env.mjData.joint("articulate_joint_1").qpos[0]
        return faucet_joint0_qpos > 0.7 or faucet_joint0_qpos <-0.7  
        

    def compute_reward(self, info: dict[str, Any], *args, **kwargs) -> float:
        faucet_is_opened = self.check_wether_faucet_is_opened(info, *args, **kwargs)
        if faucet_is_opened:
            self.reward +=0.03
        else:
            self.reward = 0.0
