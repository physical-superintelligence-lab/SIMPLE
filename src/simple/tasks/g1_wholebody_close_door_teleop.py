...
@TaskRegistry.register("g1_wholebody_close_door_teleop")
class G1WholebodyCloseDoorTaskTeleop(Task):
    uid: str = "g1_wholebody_close_door_teleop"
    label: str = "G1 TELEOP Close Door"
    description: str = (
        "A task where the G1 robot must close a door."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language=LanguageDRCfg(
            instructions=[
                "move forward to the door and close it",
            ]
        ),
        ...
        articulated = ArticulatedObjectDrCfg(
            asset_id="articulated:3", # e.g., "
            articulate_init_joint_qpos={"articulate_joint_1": random.uniform(0.0, 1.3962634015954636), "articulate_joint_2": 0.0}
        ),
        spatial=SpatialDRCfg(
            ...
            articulated_region=Box(low=[-0.1, 1,0.9], high=[-0.24, 1,0.9]),
    ...
    def reset(
        ...
        self.dr_cfgs["articulated"].articulate_init_joint_qpos = {"articulate_joint_1": random.uniform(0.0, 1.3962634015954636), "articulate_joint_2": 0.0}
    ...
    def check_wether_door_is_closed(self, info: dict[str, Any], *args, **kwargs) -> bool:
        ...
        door_joint_qpos = mujoco_env.mjData.joint("articulate_joint_1").qpos[0]
        return door_joint_qpos < -0.16
        
        

    def compute_reward(self, info: dict[str, Any], *args, **kwargs) -> float:
        door_is_closed = self.check_wether_door_is_closed(info, *args, **kwargs)
        if door_is_closed:
            self.reward +=0.03
        else:
            self.reward = 0.0
