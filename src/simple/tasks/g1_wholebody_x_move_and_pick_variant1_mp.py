...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_x_move_and_pick_variant1_mp")
class G1WholebodyXMoveAndPickVariant1MP(Task):
    uid:str = "g1_wholebody_x_move_and_pick_variant1_mp"
    label: str = "G1 Wholebody X Move and Pick Variant 1 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["21","46"]  # Exclude the target object 
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1.3, 0.15,0.0], high=[-1.4, 0.2,0.0]),

            target_region=Box(low=[-0.38, 0.26], high=[-0.45, 0.33]),
            container_region=Box(low=[-0.1, -0.03], high=[-0.17, 0.08]),
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.35, target_yaw=0, target_distance=0.7),
