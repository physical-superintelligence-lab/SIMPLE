...
@TaskRegistry.register("g1_wholebody_x_move_bend_pick_mp")
class G1WholebodyXMoveBendPickMP(Task):
    uid: str = "g1_wholebody_x_move_bend_pick_mp"
    label: str = "G1 Wholebody X Move Bend Pick MP"
    description: str = (
        "A task where the G1 robot must move to a target position and then bend and pick up an object."
    )
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-1, -0.0, 0.35], high=[-1.1, 0.0, 0.35]),
            target_region=Box(low=[-0.25, -0.02], high=[-0.28, 0.07]),
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk to target", vx=0.35, target_yaw=0, target_distance=0.54),
            StandSpec("stand", steps=30),
