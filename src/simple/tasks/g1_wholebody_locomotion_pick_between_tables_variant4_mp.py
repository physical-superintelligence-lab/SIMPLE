...
_PLACE_HEIGHT = 0.2
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant4_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant4MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant4_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 4 MP"
    ...
    sensor_cfgs: dict[str, SensorCfg] = dict(
        front_stereo = StereoCameraCfg(
            ...
            baseline=0.055,
            pose=dict( 
                position=[1.2, 0, 1.475],
                eulers=np.deg2rad([0, 70, -90]),
            )
        ),
        wrist = CameraCfg(
            uid="Logitech_C930e",
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["14","12","46","6"]  # Exclude the target object 
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[1, -0.5], high=[1.02, -0.55]),
            robot_orientation_region=Box(low=[0.717, 0, 0, 0.717], high=[0.717, 0, 0, 0.717]),
    
            # robot_region=Box(low=[-1.15, 0.0,], high=[-1.17, 0.0]),
            target_region=Box(low=[1.03, -0.15], high=[1.08, -0.18]),
            container_region=Box(low=[2.1, -0.14], high=[2.1, -0.08]),
            ...
            target_rotate_z = Box(low=1.54, high=1.59),
            obj_surface_map={
                "target": "table",
                "container": "table2",
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene8",
    ...
    def compute_reward(self, info: dict[str, Any] ,*args, **kwargs) -> float:
        is_object_on_table2 = self.check_object_on_table2(*args, **kwargs)
    ...
    def decompose(self):
        ...
        # container_pos = [1,-1.35]
        # place_origon = Box(low=[-0.1, -0.1], high=[0.1, -0.1])
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=-1.57, target_distance=0.03),
