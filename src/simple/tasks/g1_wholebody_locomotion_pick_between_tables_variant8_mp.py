...
_PLACE_HEIGHT = 0.2
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant8_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant8MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant8_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 8 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        target = TargetDRCfg(
            asset_id="graspnet1b:17" # e.g., "primitive:cube"
        ),
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["0","17","46","6"]  # Exclude the target object 
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.1, -0.45], high=[0.1, -0.4]),
            robot_orientation_region=Box(low=[0.717, 0, 0, 0.717], high=[0.717, 0, 0, 0.717]),

            target_region=Box(low=[0.14, -0.1], high=[0.16, -0.06]),
            container_region=Box(low=[1.06, 3.1], high=[1.15, 3.15]),
            container_rotate_z = Box(low=0, high=0),
            
            distractors_region=[
                Box(low=[0.4, 0.1], high=[0.6, 0.4]),  # distractor_0 on table1
                Box(low=[-0.1, 0.1], high=[0.6, 0.4]),  # distractor_1 on table1
                Box(low=[-0.1, -0.3], high=[0.6, 0.3]),  # distractor_2 on table1
                Box(low= [0.6,3.1], high=[1.2,3.4])   # distractor_3 on table2
            ],
            target_stable_indices = [0],
            target_rotate_z = Box(low=1.57, high=1.59),
            obj_surface_map={
                "target": "table",
                "container": "table2",
        ...
        scene = TabletopSceneDRCfg(
            ...
            table_height=Box(low=0.0, high=0.0),
            table2_height=Box(low=0.15, high=0.15),
            room_choices=["hssd:scene22"],
            scene_manager="hssd",
            enable_table2=True,
        ),
    ...
    def __init__(
        ...
        scene_uid: str | Scene = "hssd:scene16",
       ...
       # lazy init instance variables
        ...
        #HACK This task only in scene16
    ...
    def compute_reward(self, info: dict[str, Any] ,*args, **kwargs) -> float:
        is_object_in_container = self.check_object_on_table2(info=info, *args, **kwargs)
    ...
    def decompose(self):
        ...
        container_pos = [0.1,-2.2,0]
        place_origon = Box(low=[-0.02, -0.05], high=[0.05, 0.05])
        place_position = container_pos + np.array(place_origon.sample()+ [_PLACE_HEIGHT])

        import transforms3d as t3d
        place_orientation = t3d.euler.euler2quat(0, 0, 80)
        ...
        return [
            ...
            TurnSpec("turn_90", vx=0.1, target_yaw=0),
            TurnSpec("turn_180", vx=0.1, target_yaw=-1.57),
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=-1.57, target_distance=1.2),
