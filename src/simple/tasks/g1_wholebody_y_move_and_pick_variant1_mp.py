...
_PLACE_HEIGHT = 0.2
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_y_move_and_pick_variant1_mp")
class G1WholebodyYMoveAndPickVariant1MP(Task):
    uid:str = "g1_wholebody_y_move_and_pick_variant1_mp"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[1.02, 0.6,0.0], high=[1.04, 0.65,0.0]),
            robot_orientation_region=Box(low=[0, 0, 0, 1], high=[0, 0, 0, 1]),

            target_region=Box(low=[0.58, 0.2], high=[0.6, 0.25]),
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk to target",vx=0.1,vy=0.35, target_yaw=0, target_distance=0.45),
