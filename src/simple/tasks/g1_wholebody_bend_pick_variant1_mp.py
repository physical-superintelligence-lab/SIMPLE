...
_PLACE_HEIGHT = 0.5
_LOWER_HEIGHT = 0.1
@TaskRegistry.register("g1_wholebody_bend_pick_variant1_mp")
class G1WholebodyBendPickVariant1MP(Task):
    uid:str = "g1_wholebody_bend_pick_variant1_mp"
    label: str = "G1 Wholebody Bend Pick Variant 1 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "bend the robot and pick up the object",
            ]
        ),
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.7, -0.0,0.38], high=[-0.76, 0.0,0.38]),

            target_region=Box(low=[-0.39 ,-0.08], high=[-0.42, -0.04]),
