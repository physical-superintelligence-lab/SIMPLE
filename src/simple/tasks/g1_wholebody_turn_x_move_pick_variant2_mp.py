...
_PLACE_HEIGHT = 0.45
_LOWER_HEIGHT = 0.05
@TaskRegistry.register("g1_wholebody_turn_x_move_and_pick_variant2_mp")
class G1WholebodyTurnXMoveAndPickVariant2MP(Task):
    uid:str = "g1_wholebody_turn_x_move_and_pick_variant2_mp"
    label: str = "G1 Wholebody Turn X Move and Pick Variant 2 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["12"]  # Exclude the target object 
        ),
        ...
        spatial = SpatialDRCfg(
            ...
            robot_orientation_region = Box(
                low = np.array([0.96592583, 0.0, 0.0, -0.7], dtype=np.float32),
                high= np.array([1.00000000, 0.0, 0.0,  0.7], dtype=np.float32)
            ),

            target_region=Box(low=[-0.28,-0.17], high=[-0.32, -0.12]),
