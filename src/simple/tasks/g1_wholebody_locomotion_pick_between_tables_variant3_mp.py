...
_PLACE_HEIGHT = 0.35
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant3_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant3MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant3_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 3 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0, 0.7,], high=[0, 0.75]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[-0.04, 0.37], high=[0.00, 0.4]),
            container_region=Box(low=[0.42, 2.6], high=[0.48, 2.62]),
            ...
            distractors_region=[
                ...
                Box(low= [-0.6,2.5], high=[0.6,2.7])   # distractor_3 on table2
            ],
    ...
    def decompose(self):
        ...
        place_origon = Box(low=[-0.2, -0.1], high=[-0.05, -0.05])
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=1.57, target_distance=1.35),
