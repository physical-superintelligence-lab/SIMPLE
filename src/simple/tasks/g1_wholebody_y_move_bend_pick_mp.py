...
@TaskRegistry.register("g1_wholebody_y_move_bend_pick_mp")
class G1WholebodyYMoveBendPickMP(Task):
    uid: str = "g1_wholebody_y_move_bend_pick_mp"
    label: str = "G1 Wholebody Y Move Bend Pick MP"
    description: str = (
        "A task where the G1 robot must move to a target position and pick a box from the end of table."
    )
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        container=TargetDRCfg(asset_id="graspnet1b:46"),  # or used "graspnet1b:6"
        ...
        spatial=SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[0.4, 0.78, 0.35], high=[0.45, 0.85, 0.35]),
            robot_orientation_region=Box(
                low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]
            ),
            target_region=Box(low=[0.0, 0.43], high=[0.05, 0.48]),
            container_region=Box(low=[-0.3, -0.03], high=[-0.37, 0.08]),
            distractors_region=Box(low=[0.3, -0.1], high=[-0.5, 0.3]),
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # HACK This task only in scene18
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec(
                "walk to target", vx=0.1, vy=-0.35, target_yaw=0, target_distance=0.66
            ),
