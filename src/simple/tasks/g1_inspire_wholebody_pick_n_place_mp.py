...
_LIFT_HEIGHT = 0.04
@TaskRegistry.register("g1_inspire_wholebody_pick_n_place_mp")
class G1InspireWholebodyPickNPlaceMP(Task):
    uid:str = "g1_inspire_wholebody_pick_n_place_mp"
    label: str = "G1 Inspire Wholebody Pick N Place Task"
    description: str = "A task where the G1 Inspire robot must pick up an object and place it on a target location."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language = LanguageDRCfg(
            instructions = [
                "from the start position, pick up the object and place it on the target position.",
            ]
        ),
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.9, 0.0, 0.0], high=[-0.9, 0.0, 0.0]),
            target_region=Box(low=[-0.5, 0.05], high=[-0.5, 0.05]),
    ...
    def __init__(
        self, 
        robot_uid: str = "g1_inspire_wholebody",
       ...
       # lazy init instance variables
        ...
        #HACK This task only use scene3
    ...
    def observation_space(self) -> spaces.Space:
        ...
        obs: dict[str, Any] = {
            ...
            # "head_stereo_left": spaces.Box(0, 255, shape=(360, 640, 3), dtype=np.uint8),
            # "head_stereo_right": spaces.Box(0, 255, shape=(360, 640, 3), dtype=np.uint8),
    ...
    def compute_reward(self, info: dict[str, Any], *args, **kwargs) -> float:
        is_object_in_destination = self.check_object_in_destination(*args, **kwargs)
       
        if is_object_in_destination:
            self.reward += 0.1
        else:
            self.reward = 0
        return self.reward
    ...
    def decompose(self):
        ...
        return [
            ...
            GraspObjectSpec("approach", target_uid=self.target.uid, pregrasp=False, grasp_type="bodex", 
                            hand_uid="inspire_right", lock_links=["waist_roll_link", ]),    # "torso_link", "L_hand_base_link"
            LiftSpec("lift", up=0.08, grasp_type="bodex", hand_uid="inspire_right"),

            TurnSpec("turn", vx=0.1, target_yaw=1.57),
            WalkSpec("walk", vx = 0.5, vy=0, target_yaw=1.57, target_distance=0.7),
            TurnSpec("turn", vx=0.5, target_yaw=0),
            WalkSpec("walk", vx = 0.5, vy=0, target_yaw=0, target_distance=2.8),
