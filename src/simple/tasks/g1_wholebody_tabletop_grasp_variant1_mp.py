...
_LIFT_HEIGHT = 0.02
@TaskRegistry.register("g1_wholebody_tabletop_grasp_variant1_mp")
class G1WholebodyTabletopGraspVariant1MP(Task):
    uid:str = "g1_wholebody_tabletop_grasp_variant1_mp"
    label: str = "G1 Wholebody Tabletop Grasp Variant 1 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[-0.2, 0.8,0.0], high=[-0.2, 0.8,0.0]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[-0.23, 0.43], high=[-0.18, 0.46]),
