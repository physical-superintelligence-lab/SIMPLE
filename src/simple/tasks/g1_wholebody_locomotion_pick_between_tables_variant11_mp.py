...
_PLACE_HEIGHT = 0.2
@TaskRegistry.register("g1_wholebody_locomotion_pick_between_tables_variant11_mp")
class G1WholebodyLocomotionPickBetweenTablesTaskVariant11MP(Task):
    uid:str = "g1_wholebody_locomotion_pick_between_tables_variant11_mp"
    label: str = "G1 Wholebody Locomotion Pick and Place variant 11 MP"
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        ...
        distractors = DistractorDRCfg(
            ...
            exclude=["14","46","6"]  # Exclude the target object 
        ),

        spatial = SpatialDRCfg(
            spatial_mode="random",
            robot_region=Box(low=[1.1, 1.4], high=[1.1, 1.45]),
            robot_orientation_region=Box(low=[0.717, 0, 0, -0.717], high=[0.717, 0, 0, -0.717]),

            target_region=Box(low=[1.06, 1.06], high=[1.12, 1.08]),
            container_region=Box(low=[1.06, 4.1], high=[1.15, 4.15]),
            ...
            distractors_region=[
                ...
                Box(low= [0.6,4], high=[1.2,4.4])   # distractor_3 on table2
            ],
            target_stable_indices = [0],
            target_rotate_z = Box(low=-1.57, high=-1.57),
            obj_surface_map={
                "target": "table",
                "container": "table2",
    ...
    def decompose(self):
        ...
        return [
            ...
            WalkSpec("walk_to_table2", vx=0.35, vy=0.15, target_yaw=1.57, target_distance=0.9),
