...
@TaskRegistry.register("g1_wholebody_open_oven_teleop")
class G1WholebodyOpenOvenTaskTeleop(Task):
    uid: str = "g1_wholebody_open_oven_teleop"
    label: str = "G1 TELEOP Open Oven"
    description: str = (
        "A task where the G1 robot must open the oven."
    ...
    dr_cfgs: dict[str, RandomizerCfg] = dict(
        language=LanguageDRCfg(
            instructions=[
                "move forward to the oven and open it",
            ]
        ),
    ...
    def check_wether_oven_is_opened(self, info: dict[str, Any], *args, **kwargs) -> bool:
        ...
        oven_joint2_qpos = mujoco_env.mjData.joint("articulate_joint_2").qpos[0]
        oven_joint1_qpos = mujoco_env.mjData.joint("articulate_joint_1").qpos[0]
        return oven_joint2_qpos > 1.4 or oven_joint1_qpos >1.4  
        
        

    def compute_reward(self, info: dict[str, Any], *args, **kwargs) -> float:
        oven_is_opened = self.check_wether_oven_is_opened(info, *args, **kwargs)
        if oven_is_opened:
            self.reward +=0.03
        else:
            self.reward = 0.0
