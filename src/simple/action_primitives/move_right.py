...
def move_right(robot, arm='left', distance_cm=3.0, frame='world', camera_pose=None):
    """
    Move the robot's end-effector to the right by a specified distance using curobo IK.
    
    This action primitive uses curobo's IK solver to compute the joint positions
    needed to move the end-effector left while maintaining its orientation.
    ...
    if frame == 'world':
        ...
        if arm == 'left':
            target_ee_pos = left_ee_pos + np.array([0.0, -distance_m, 0.0])
            target_ee_quat = left_ee_quat
        else:
            target_ee_pos = right_ee_pos + np.array([0.0, -distance_m, 0.0])
