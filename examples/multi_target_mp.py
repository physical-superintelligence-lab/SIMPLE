...
def main(
    ...
    if not os.path.exists(PRERESET_TASK_STATE):
        state_dict = task.state_dict()
        dump_json(state_dict, open(PRERESET_TASK_STATE, "w"), indent=4)
    ...
    with tqdm(total=0, bar_format='{desc}', ncols=80, file=sys.stdout) as t:
        ...
        camera_spinner = itertools.cycle(['translate_left', 'translate_up', 'translate_right', 'translate_down'])
        while not episode_over:
            ...
            try:
                ...
                vlm_agent.move_camera(dir, distance=0.05, step=0.01)
                action = vlm_agent.get_action(observation, info)
